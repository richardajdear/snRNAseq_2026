...
def project_combined_postnatal_EN_100k_thesis():
    
    ad_backed = sc.read_h5ad('/home/rajd2/rds/rds-cam-psych-transc-Pb9UGUlrwWc/Cam_snRNAseq/combined/combined_postnatal_full_harmony_EN_100k.h5ad', backed='r')
    
    # Filter for prefrontal cortex in backed mode
    pfc_regions = ['BA8', 'BA9', 'BA10', 'BA11', 'BA46', 'BA9/46', 'PFC', 'FC', 'FIC', 'dorsolateral prefrontal cortex', 'DLPFC', 'Frontoparietal cortex']
    mask = ad_backed.obs['region'].isin(pfc_regions)
    print(f"Filtering {mask.sum()} / {ad_backed.shape[0]} cells for Prefrontal Cortex")
    
    ad = ad_backed[mask].to_memory()
    
    # Ensure `.X` is fully in memory and not a backed view
    if hasattr(ad.X, 'copy'):
        ad.X = ad.X.copy()
    ...
    out_path = '/home/rajd2/rds/hpc-work/snRNAseq_2026/notebooks/ahbaC3_projection/projection_results_combined_postnatal_EN_100k_thesis.csv'
    out_df.to_csv(out_path)
    print("Saved exact thesis replication to", out_path)

if __name__ == '__main__':
    project_combined_postnatal_EN_100k_thesis()
