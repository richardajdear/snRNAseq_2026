...
Pipeline steps:
    ...
    umap        — compute UMAP from scANVI latent space (skipped if already stored)
...
from .kernels import bin_ages, build_kernels, ensure_neighbors, ensure_umap, run_moscot_ot, validate_scanvi_outputs
from .plots import (
    plot_coarse_transition_matrix,
    plot_excitatory_l23_fate_umap,
    plot_excitatory_l23_pca_fate,
...
def run(config: CellRankConfig) -> ad.AnnData:
    ...
    # ── SCANVI VALIDATION ──────────────────────────────────────────────────────
    # Always validate that scANVI was run before proceeding with any step.
    # This provides a clear, early error rather than a confusing failure later.
    logger.info("─" * 40)
    logger.info("Validating scANVI outputs...")
    validate_scanvi_outputs(adata, config, logger)
    ...
    if "umap" in steps:
        ...
        ensure_umap(adata, config, logger)
        log_memory("After umap", logger)
    ...
    if "fate_probs" in steps:
        ...
        if config.save_plots:
            ...
            plot_excitatory_l23_pca_fate(adata, g, config, logger)
