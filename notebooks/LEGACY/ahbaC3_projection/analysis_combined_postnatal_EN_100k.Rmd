...
```{r define_vars}
# Define paths
input_raw <- "/home/rajd2/rds/rds-cam-psych-transc-Pb9UGUlrwWc/Cam_snRNAseq/combined/combined_postnatal_full_harmony_EN_100k.h5ad"
output_csv <- "projection_results_combined_postnatal_EN_100k_thesis.csv"
...
# Rename donor column to Individual early for summary stats
if (!"Individual" %in% colnames(data)) {
  if ("donor_id" %in% colnames(data)) {
    data <- data %>% rename(Individual = donor_id)
  } else if ("individual" %in% colnames(data)) {
    data <- data %>% rename(Individual = individual)
  }
}

if (!("cell_class" %in% colnames(data)) && "lineage" %in% colnames(data)) {
  data <- data %>% rename(cell_class = lineage)
}
if (!("cell_class" %in% colnames(data))) {
  data$cell_class <- "Unknown"
}
...
age_summary <- data %>%
  ...
  summarize(
    n_cells = n(),
    n_donors = n_distinct(Individual),
...
data <- data %>%
  select(-any_of(c("Age_log2", "age_range"))) %>%
  rename(any_of(c(Age_log2 = "age_log2", age_range = "age_category")))
...
plot_data <- data %>%
  filter(str_detect(cell_class, "Excitatory") | str_detect(cell_type, "Excitatory") | str_detect(cell_type, "neural cell")) %>%
