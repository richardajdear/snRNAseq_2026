---
title: "AHBA C3 Age Sensitivity Analysis (Gap Model)"
format: md
jupyter: python3
---
...
CHILD_START  = _cfg.get('CHILD_START', 1)
CHILD_ENDS   = _cfg.get('CHILD_ENDS',  [7, 8, 9, 10])
ADOL_STARTS  = _cfg.get('ADOL_STARTS', [10, 11, 12, 13, 14, 15])
ADOL_ENDS    = _cfg.get('ADOL_ENDS',   [19, 21, 23, 25])
...
We fix the childhood lower bound at `CHILD_START` and independently vary:

- **Childhood upper bound**: controlled by `CHILD_ENDS`
- **Adolescence lower bound**: controlled by `ADOL_STARTS`
- **Adolescence upper bound**: controlled by `ADOL_ENDS`
...
```{python}
%%R -i final_df -i code_dir -i N_VALUES -i CHILD_START -i CHILD_ENDS -i ADOL_STARTS -i ADOL_ENDS
